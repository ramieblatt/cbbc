class Player < ApplicationRecord
  ...
  scope :with_career_num_ips_greater_than, -> (tot_ips) { Player.with_career_num_ipouts_greater_than(tot_ips.to_i*3) }
  scope :with_career_num_ips_less_than, -> (tot_ips) { Player.with_career_num_ipouts_less_than(tot_ips.to_i*3) }
  ...
  def self.apply_prebuilt_search(query)
    puts "query:"
    puts query.inspect
    p = Player
    query.each do |k,v|
      if v.presence
        puts "about to send(\"#{k}\", #{v})"
        p = p.send(k, v)
      else
        puts "about to send(\"#{k}\")"
        p = p.send(k)
      end
    end
    p
